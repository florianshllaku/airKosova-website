...
function renderFlights(data) {
  ...
  function parsePrice(text) {
    ...
    if (!m) return { currency: null, price: null };
    const cur = (m[1] === '€' || m[2] === '€') ? 'EUR' : String(m[1] || m[2] || '').toUpperCase();
    const numRaw = String(m[2] || m[1] || '').trim();
  ...
  function renderFlightCard(f, leftName, leftCode, rightName, rightCode) {
    const price =
      f.soldOut ? 'Sold out' :
      (f.price ? `${currencySymbol(f.currency)} ${f.price}` : (f.priceText || '—'));
    const dur = f.duration ? f.duration : '—';
    const fn = f.flightNumber ? String(f.flightNumber) : '';
    return `
      <div class="flight-card">
        ...
        <div class="fc-line">
          ...
          ${fn ? `<div class="fc-flightno">Flight: <strong>${escapeHtml(fn)}</strong></div>` : ''}
        </div>
        ...
        <div class="fc-price${f.soldOut ? ' sold' : ''}">${escapeHtml(price)}</div>
      </div>
    `;
  }
  ...
  const renderOutbound = (key) => {
    ...
    const content =
      (flights.length
        ? flights.map((f) => renderFlightCard(f, leftName, fromCode, rightName, toCode)).join('')
        : (anyOutbound ? '' : renderSimpleRowsTable(outbound.rawRows)));

    if (outCardsHost) outCardsHost.innerHTML = content;
  };
  ...
  const renderReturn = (key) => {
    ...
    const content =
      (flights.length
        ? flights.map((f) => renderFlightCard(f, leftName, toCode, rightName, fromCode)).join('')
        : (anyReturn ? '' : renderSimpleRowsTable(inbound.rawRows)));

    if (retCardsHost) retCardsHost.innerHTML = content;
  };
